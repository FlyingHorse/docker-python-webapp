#!/usr/bin/env python
# -*- coding: utf-8 -*-
# @Date    : 2016-06-25 15:36:43
# @Author  : Your Name (you@example.org)
# @Link    : http://example.org
# @Version : $Id$

import os, types, re, cgi, sys, time, datetime, functools, mimetypes, logging, urllib, traceback, json
...
class RedirectError(HttpError):
    ...
    def __str__(self):
        return '%d %s' % (self.status, self.location)
...
class Response(object):
    ...
    def headers(self):
        ...
        if hasattr(self, '_cookies'):
            logging.info('cookies in resp headers: %s' % self._cookies)
            for v in self._cookies.itervalues():
                L.append(('Set-Cookie', v))
        L.append(_HEADER_X_POWERED_BY)
        logging.info('resp headers: %s' % L)
        return L
    ...
    def set_cookie(self, name, value, max_age=None, expires=None, path='/', domain=None, secure=False, http_only=True):
        ...
        logging.info('cookie %s' % self._cookies)
